...
| File | Description |
|------|-------------|
| `package.json` | Node.js project config - dependencies (axios, cheerio), scripts |
...
- Full workflow: check count → validate company (ANAF+Peviitor) → scrape Ciklum → transform (fix locations) → upsert → log summary
