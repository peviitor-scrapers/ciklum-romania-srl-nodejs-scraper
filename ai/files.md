...
| `job-validator.js` | Shared validation primitives - exports validateByHead(url), validateByContent(url, opts), DEFAULT_EXPIRED_KEYWORDS. Used by both `validate-jobs.js` and `tests/validate-ciklum-jobs.js`. |
...
| `tests/e2e/scraper.test.js` | E2E tests - full pipeline with real Ciklum API and ANAF |
...
- Full workflow: validate company (ANAF+Peviitor) → scrape Ciklum → transform → upsert via API → generate docs/jobs.md